...
function initialize(){
  ...
  dbInit.initialize(params.database.alphaquadrant, pjson.name, function() {
    modules.initialize(params);

    app.listen(params.application_port.alpha_quadrant, function () {
        console.log('Server gestart op poort ' + params.application_port.alpha_quadrant);
    });
